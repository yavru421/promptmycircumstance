...
$oldProjectFile = Join-Path $Destination "PourAndMeasure.csproj"
...
$filesToProcess = Get-ChildItem -Path $Destination -Include *.cs, *.razor, *.csproj, *.html, *.json, *.toml, *.md, *.ps1, *.sh, _headers -Recurse -File
...
foreach ($file in $filesToProcess) {
    ...
    if ($content -match "PourAndMeasure" -or $content -match "pourandmeasure") {
        # Case sensitive replacements
        $content = $content -replace "PourAndMeasure", $NewAppName
        $content = $content -replace "pourandmeasure", $NewAppName.ToLower()
